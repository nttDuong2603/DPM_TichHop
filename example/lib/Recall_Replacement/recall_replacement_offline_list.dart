...
class OfflineRecallReplacemantList extends StatefulWidget {
  ...
  @override
  State<OfflineRecallReplacemantList> createState() => OfflineRecallReplacemantListState();
}
...
class OfflineRecallReplacemantListState extends State<OfflineRecallReplacemantList> {
  ...
  void _navigateToCreateCalendarRecall(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => CreateCalendarRecallReplacement(taiKhoan: widget.taiKhoan!)),
    );
  }
  ...
  void _navigateToHistoryCalendarRecall(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => HistoryRecallReplacement(taiKhoan: widget.taiKhoan!)),
    );
  }
  void _navigateToOfflineRecallManageDeleted(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => OfflineRecallReplacemantListDeleted(taiKhoan: widget.taiKhoan!)),
    );
  }
  ...
  Future<void> _initializeEventList() async {
    var events = await CalendarRecallReplacementDatabaseHelper().getEvents(widget.taiKhoan!);
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return
                          Dismissible(
                              ...
                              direction: event.soluongquetTT <= 0 ? DismissDirection.endToStart : DismissDirection.none,
                                ...
                                if (direction == DismissDirection.endToStart) {
                                  if (event.soluongquetTT <= 0) {
                                    ...
                                    await Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (context) =>
                                            EditRecallReplacementCalendarPage(event: event, onUpdateEvent: updateEvent),
                                      ),
                                    );
                              ...
                              child: GestureDetector(
                                onTap: () async {
                                  final result = await Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                      builder: (context) => SendDataRecallReplacement(
                                        event: event,
