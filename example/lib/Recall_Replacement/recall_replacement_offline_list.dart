...
import 'calendar_recall_replacement.dart';
import 'history_recall_replacement.dart';
import 'calendar_recall_replacement_deleted.dart';
import 'recall_replacement_edit.dart';
import 'recall_replacement_send_data.dart';

class OfflineRecallReplacemantList extends StatefulWidget {
  final String taiKhoan;

  const OfflineRecallReplacemantList({
    Key? key,
    required this.taiKhoan,
  }) : super(key: key);

  @override
  State<OfflineRecallReplacemantList> createState() =>
      OfflineRecallReplacemantListState();
}

class OfflineRecallReplacemantListState
    extends State<OfflineRecallReplacemantList> {
  ...
  void _navigateToCreateCalendarRecall(BuildContext context) {
    Navigator.push(
      ...
      MaterialPageRoute(
          builder: (context) =>
              CreateCalendarRecallReplacement(taiKhoan: widget.taiKhoan!)),
    );
  }
  ...
  void _navigateToHistoryCalendarRecall(BuildContext context) {
    Navigator.push(
      ...
      MaterialPageRoute(
          builder: (context) =>
              HistoryRecallReplacement(taiKhoan: widget.taiKhoan!)),
    );
  }
  ...
  void _navigateToOfflineRecallManageDeleted(BuildContext context) {
    Navigator.push(
      ...
      MaterialPageRoute(
          builder: (context) =>
              OfflineRecallReplacemantListDeleted(taiKhoan: widget.taiKhoan!)),
    );
  }
  ...
  Widget _build() {
    switch (selectIndex) {
      case 0:
        ...
        return CreateCalendarRecallReplacement(taiKhoan: widget.taiKhoan);
      case 1:
        // Trang "Lịch sử phân phối"
        return HistoryRecallReplacement(taiKhoan: widget.taiKhoan);
      case 2:
        // Trang "Lịch đã xóa"
        return OfflineRecallReplacemantListDeleted(taiKhoan: widget.taiKhoan);
      default:
  ...
  Future<void> _initializeEventList() async {
    var events = await CalendarRecallReplacementDatabaseHelper()
  ...
  void updateEventList(CalendarRecallReplacement deletedEvent) {
    if (_eventListFuture != null) {
      setState(() {
        _eventListFuture = _eventListFuture!.then((eventList) {
          eventList
              .removeWhere((event) => event.idLTHTT == deletedEvent.idLTHTT);
          return eventList;
        });
      });
    }
    ...
    return WillPopScope(
        onWillPop: () async {
          ...
          Navigator.pushAndRemoveUntil(
            ...
            // Giả sử bạn truyền taiKhoan vào HomePage
            (Route<dynamic> route) =>
                false, // Xóa tất cả các routes khác khỏi stack
          );
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        if (eventList.isEmpty) {
                          ...
                          eventList.sort((a, b) =>
                              DateTime.parse(b.ngayTaoLTHTT)
                                  .compareTo(DateTime.parse(a.ngayTaoLTHTT)));
                          return ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return Dismissible(
                                  ...
                                  direction: event.soluongquetTT <= 0
                                      ? DismissDirection.endToStart
                                      : DismissDirection.none,
                                  confirmDismiss: (direction) async {
                                    if (direction ==
                                        DismissDirection.endToStart) {
                                      if (event.soluongquetTT <= 0) {
                                        // Thực hiện chuyển hướng tới trang EditPackageCalendarPage
                                        await Navigator.push(
                                          context,
                                          MaterialPageRoute(
                                            builder: (context) =>
                                                EditRecallReplacementCalendarPage(
                                                    event: event,
